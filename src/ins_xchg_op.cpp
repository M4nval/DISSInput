...
static void PIN_FAST_ANALYSIS_CALL _cmpxchg_r2m_opw_slow(THREADID tid,
  ...
  tag_t src_tags[] = {RTAG[src][0], RTAG[src][1]};
  for (size_t i = 0; i < 2; i++) {
    r2m_op(dst + i, src_tags[i], (i==1?false:true));
  }
}
...
static void PIN_FAST_ANALYSIS_CALL _xchg_m2r_opq(THREADID tid, uint32_t dst,
  ...
  r2m_op(src + 2, tmp_tag[2], true);
  r2m_op(src + 3, tmp_tag[3], true);
  r2m_op(src + 4, tmp_tag[4], true);
  r2m_op(src + 5, tmp_tag[5], true);
  r2m_op(src + 6, tmp_tag[6], true);
  r2m_op(src + 7, tmp_tag[7], false);
}
