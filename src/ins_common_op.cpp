...
void r2m_op(ADDRINT dst, tag_t src_tag_id, bool hasNext){
  ...
  tag_entity* next_tag = tag_get(MTAG(dst+1));

  ADDRINT firstAddr = dst;
  ADDRINT finalAddr = dst + 1;

  if (pre_tag && pre_tag->parent == src_tag->parent && src_tag->begin == pre_tag->end){
    firstAddr = getFirstAddr(dst, pre_tag->id);
    LOGD("[r2m taint!][combined with pre-tag]  lhs=%s, rhs=%s\n", tag_sprint(pre_tag).c_str(), tag_sprint(src_tag).c_str());
    src_tag = tag_combine(pre_tag, src_tag);
  }
  if (!hasNext && next_tag && next_tag->parent == src_tag->parent && src_tag->end == next_tag->begin){
    finalAddr = getFinalAddr(dst+1, next_tag->id);
    LOGD("[r2m taint!][combined with next-tag]  lhs=%s, rhs=%s\n", tag_sprint(src_tag).c_str(), tag_sprint(next_tag).c_str());
    src_tag = tag_combine(src_tag, next_tag);
  }
  ...
  LOGD("[r2m taint!]  dst=%p, src_tag=%s, pre_tag=%s, next_tag=%s, updateAddr=[%p,%p)\n", 
